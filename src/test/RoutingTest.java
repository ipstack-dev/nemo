...
public class RoutingTest {
	...
	private static void testNetwork(String type, int n, long bit_rate, IpPrefix net_prefix, long count, int pg_size, boolean print_routing_table) {
		Network<Node<IpAddress,Packet<IpAddress>>,IpLink<IpAddress,Packet<IpAddress>>> network;
		if (type.toLowerCase().startsWith("alt")) {
			network=TopologyBuilder.linearIpNetworkAlt(n,bit_rate,net_prefix);
		}
		else
		if (type.toLowerCase().startsWith("lin")) {
			network=TopologyBuilder.linearIpNetwork(n,bit_rate,net_prefix);
		}
		else
		if (type.toLowerCase().startsWith("man")) {
			network=TopologyBuilder.manhattanIpNetwork(n,n,bit_rate,net_prefix);
		}
		else
		if (type.toLowerCase().startsWith("tree")) {
			int degree=type.length()>4? Integer.parseInt(type.substring(4)) : 2;
			network=TopologyBuilder.treeIpCoreNetwork(degree,n,bit_rate,net_prefix);
		}
		...
		Collection<IpLink<IpAddress,Packet<IpAddress>>> links=network.getLinks();
		ArrayList<IpLink> access_links=new ArrayList<>();
		for (IpLink link : links) if (!(link.getPrefix().equals(TopologyBuilder.CORE_LOCAL_LINK_PREFIX4) || link.getPrefix().equals(TopologyBuilder.CORE_LOCAL_LINK_PREFIX6))) access_links.add(link);
		if (VERBOSE) System.out.println("Access links: "+access_links.toString());

		// SRC point
		IpLink link1=access_links.get(0);
		...
		if (VERBOSE) System.out.println("Source: h1="+h1_addr+"/"+prefix1.getPrefixLength()+", gw="+r1_addr);
		
		// DST point
		IpLink link2=access_links.get(access_links.size()-1);
		...
		if (VERBOSE) System.out.println("Target: h2="+h2_addr+"/"+prefix2.getPrefixLength()+", gw="+r2_addr);
		
		if (print_routing_table) {
			System.out.println("R0-RT:\n"+((RoutingTable)((Node)nodes[0]).getRoutingFunction()).toStringWithSpaces()+"\n");
			int last=nodes.length-1;
			if (last>0) System.out.println("R"+last+"-RT:\n"+((RoutingTable)((Node)nodes[last]).getRoutingFunction()).toStringWithSpaces()+"\n");
		}
		...
		if (pg_size<=0) {
			...
			System.out.print(""+n
					+'\t'+nodes.length
					+'\t'+(network.getLinks()!=null?access_links.size():0)
					+'\t'+pg.getHopNumber()
					+'\t'+pg.getRxCount()
	...
	public static void main(String[] args) throws InterruptedException {
		...
		long bit_rate=DateFormat.parseLongKMG(flags.getString("-b",String.valueOf(DEFAULT_BIT_RATE),"<bit-rate>","link capacity [b/s] (default is "+DateFormat.formatBitRate(DEFAULT_BIT_RATE)+")"));
		int count=flags.getInteger("-c",3,"<count>","number of ping messages");
		int n=flags.getInteger("-n",1,"<size>","network size n (e.g. manhattan nxn or tree height n)");
		int N=flags.getInteger("-N",n,"<size>","maximum network size n");
		String type=flags.getString("-t","alt","<type>","network type (manhattan, linear, tree, tree3, tree4, etc)");
		...
		int packet_generator=flags.getInteger("-g",-1,"<size>","uses a packet generator in place of ping, with the given packet size");
		...
		if (type.toLowerCase().startsWith("tree")) {
			int degree=type.length()>4? Integer.parseInt(type.substring(4)) : 2;
			if (N==n) System.out.println("Tree network, degree="+degree+", height="+n+", routers="+(int)(Math.pow(degree,n+1)-1)/(degree-1)+", links="+(int)((Math.pow(degree,n+1)-1)/(degree-1)-1+Math.pow(degree,n))+", bit-rate="+DateFormat.formatBitRate(bit_rate));
			else System.out.println("Tree network, degree="+degree+", height=n, routers="+(degree==2? "2^(n+1)-1" : "("+degree+"^(n+1)-1)/"+(degree-1))+",  links="+(degree==2? "2^(n+1)-1" : "("+degree+"^(n+1)-1)/"+(degree-1))+"-1+"+degree+"^n, bit-rate="+DateFormat.formatBitRate(bit_rate));
		}
