...
public class IPv4HybridRoutingTest {
	...
	static NetInterfaceListener<EthAddress,EthPacket> eth_sniffer_listener=new NetInterfaceListener<EthAddress,EthPacket>() {
		@Override
	...
	static NetInterfaceListener<Ip4Address,Ip4Packet> ip4_sniffer_listener=new NetInterfaceListener<Ip4Address,Ip4Packet>() {
		@Override
	...
	private static void testHybridRoute(boolean tcpdump) {
		try {
			...
			// internal network
			DataLink<Ip4Address,Ip4Packet> link1=new DataLink<>(LINK_BIT_RATE);
						
			// R1
			NetInterface<Ip4Address,Ip4Packet> r1_eth0=new Ip4EthInterface(new EthLayer(new RawEthInterface("eth0")),new Ip4AddressPrefix(r1_1_1,24));
			r1_eth0.addAddress(r1_1_2);
			NetInterface<Ip4Address,Ip4Packet> r1_eth1=new DataLinkInterface(link1,r1_2);
			Ip4Layer router1=new Ip4Layer(r1_eth0, r1_eth1);
			router1.setForwarding(true);
			
			// H1 network layer
			NetInterface<Ip4Address,Ip4Packet> h1_eth0=new DataLinkInterface<Ip4Address,Ip4Packet>(link1,h1);
			Ip4Layer host1=new Ip4Layer(h1_eth0);
			//host1.getRoutingTable().add(new Route(new Ip4Prefix("0.0.0.0/0"),r1_2,h1_eth0));
			host1.getRoutingTable().add(new Route<Ip4Address,Ip4Packet>(Ip4Prefix.ANY,r1_2,h1_eth0));
			...
			if (tcpdump) {
				new MultipleNetInterface<EthAddress,EthPacket>(eth_sniffer_listener).addInterface(new RawEthInterface("eth0"));
				new MultipleNetInterface<Ip4Address,Ip4Packet>(ip4_sniffer_listener).addInterface(new PromiscuousDataLinkInterface<Ip4Address,Ip4Packet>(link1,tcpdump_addr));
			}
			...
			while (true) {
				...
				System.out.println("UDP ECHO: received data: "+Bytes.toHex(datagram_packet.getData(),datagram_packet.getOffset(),datagram_packet.getLength()));
	...
	/*private static void testLinearNetwork(int n, String r0_ext_if, Ip4AddressPrefix r0_ext_addr_prefix, ShortestPathAlgorithm algo, boolean udp_echo, boolean tcpdump, int ping_count, long ping_time) {
		...
		try {
			...
			// tcpdump
			if (tcpdump) {
				NetInterface[] pints=new PromiscuousDataLinkInterface[n];
				for (int i=0; i<n; i++) pints[i]=new PromiscuousDataLinkInterface(links[i],new Ip4Address("10."+i+".0.254"));
				new Sniffer(pints,ip4_sniffer_listener);
			}
	...
	private static void testManhattan(int n, String ext_ni, Ip4AddressPrefix ext_addr_prefix, Ip4Address ext_gw, int k, ShortestPathAlgorithm algo, boolean tcpdump, int ping_count, long ping_time) {
		...
		try {
			...
			*/
			...
			// tcpdump
			if (tcpdump) {
				/*ArrayList<NetInterface<Ip4Address,Ip4Packet>> pints=new ArrayList<>();
				for (int i=0; i<n; i++) {
					for (int j=0; j<n; j++) {
						int index=i*n+j;
						pints.add(new PromiscuousDataLinkInterface<Ip4Address,Ip4Packet>(links[2*index],new Ip4Address("10."+i+"."+j+".126")));
						pints.add(new PromiscuousDataLinkInterface<Ip4Address,Ip4Packet>(links[2*index+1],new Ip4Address("10."+i+"."+j+".254")));
					}
				}
				new Sniffer<Ip4Address,Ip4Packet>(ip4_sniffer_listener).addInterfaces(pints);*/
				MultipleNetInterface<Ip4Address,Ip4Packet> sniffer=new MultipleNetInterface<>(ip4_sniffer_listener);
				for (Link<Ip4Address,Ip4Packet> link : links) sniffer.addLink(link);
			}
			...
			if (ext_ni!=null) {
				...
				//link_interface.send(new IcmpEchoRequestMessage(local_addr_prefix,h1,0,0,"0123456789".getBytes()).toIp4Packet(),r0_1_1);
				Ip4Layer ip_layer=new Ip4Layer(new DataLinkInterface[]{link_interface});
				ip_layer.getRoutingTable().add(Ip4Prefix.ANY,ext_addr_prefix);
				System.out.println("H1 running at "+local_addr_prefix);
				PingClient ping_client=new PingClient(ip_layer,System.out);
				ping_client.ping(0,"0123456789".getBytes(),h2,ping_count,ping_time);
			}
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		int n=flags.getInteger("-n",4,"<num>","number of nodes in a row; in case of Manhattan topology the number of node is nxn");
		int k=flags.getInteger("-k",2*n*n-1,"<num>","index of the network where the host is attached");
		...
		String ext=flags.getString("-i",null,"<if/addr/len/gw>","name, IPv4 address, prefix length, gateway for the external interface of router R00 (e.g. tun0/172.16.0.2/24/172.16.0.1)");
		if (ext!=null) {
		...
		int ping_count=flags.getInteger("-c",3,"<num>","number of ping requests");
		long ping_time=flags.getLong("-t",1000,"<time>","ping inter-time [millisec]");
		String algo_name=flags.getString("-algo","sim","<algo>","shortest-path alorithm");
		
		if (help) {
			System.out.println(flags.toUsageString(IPv4HybridRoutingTest.class));
