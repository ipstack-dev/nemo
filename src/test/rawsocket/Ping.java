...
public class Ping {
	...
	public static void main(String[] args) throws SocketException {				
		...
		int count=flags.getInteger("-c",3,"<num>","sends only <num> Echo requests");
		long ping_time=flags.getInteger("-t",1000,"<time>","Echo requests are sent every <time> milliseconds");
		...
		String ipaddr_prefix=flags.getString(null,null,"<ipaddr>/<prefix-len>","local IP address and prefix length");
		String target=flags.getString(null,null,"<target>","remote IP address");		
		...
		try {
			...
			if (ping4) {
				...
				Ip4Layer ip_layer=new Ip4Layer(ip_interface);
				...
				Ip6Layer ip_layer=new Ip6Layer(ip_interface);
