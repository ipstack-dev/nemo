...
public class HttpClient {
	...
	public HttpResponse request(String method, HttpRequestURL req_url, HashMap<String,String> hdr, String content_type, byte[] body) throws IOException {
		...
		HttpRequest req=new HttpRequest(method.toUpperCase(),(abspath!=null?abspath:"/")+(query!=null?'?'+query:""),hdr,body);
		//System.out.println("DEBUG: "+this.getClass().getSimpleName()+": request(): request: "+req);
		...
		//System.out.println("DEBUG: "+this.getClass().getSimpleName()+": request(): response: "+resp);
		//System.out.println("DEBUG: "+this.getClass().getSimpleName()+": request(): response has stream: "+resp.hasStream());
		if (!resp.hasInputStream()) socket.close();	
		return resp;
	}
