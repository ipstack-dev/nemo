 ...
 * It is a {@link Packet} with a byte array payload.
 * @param <A> the address type
 */
public abstract class DataPacket<A extends Address> implements Packet<A> {
	...
	/** Source address */
	protected A src_addr=null;

	/** Destination address */
	protected A dst_addr=null;
	...
	public DataPacket(A src_addr, A dst_addr, byte[] data_buf, int data_off, int data_len) {
	...
	public DataPacket(A src_addr, A dst_addr, byte[] data) {
		this(src_addr,dst_addr,data,0,data!=null?data.length:0);
	}
	
	/** Creates a new packet.
	 * @param pkt the packet */
	protected DataPacket(DataPacket<A> pkt) {
	...
	public Object clone() {
		try {
			...
			DataPacket<A> pkt=(DataPacket<A>)super.clone();
