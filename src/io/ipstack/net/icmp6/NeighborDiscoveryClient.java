...
import io.ipstack.net.icmp6.message.option.SourceLinkLayerAddressOption;
...
public class NeighborDiscoveryClient {
	...
	public NeighborDiscoveryClient(Ip6EthInterface ip_interface, EthAddress eth_addr, long arp_table_timeout) {
	...
	public NeighborDiscoveryClient(Ip6EthInterface ip_interface, EthAddress eth_addr, long arp_table_timeout, Ip6Address ip_addr) {
		this(ip_interface,eth_addr,arp_table_timeout);
	...
	public synchronized EthAddress request(Ip6Address target_ip_addr) {
		Ip6Address local_ip_addr=null;
		synchronized (ip_addrs) {
			for (Ip6Address addr : ip_addrs) {
				if (addr instanceof Ip6AddressPrefix && ((Ip6AddressPrefix)addr).getPrefix().contains(target_ip_addr)) {
					local_ip_addr=addr;
					break;
				}
			}
		...
		try {
			...
			Icmp6NeighborSolicitationMessage ns_msg=new Icmp6NeighborSolicitationMessage(local_ip_addr,new SolicitedNodeMulticastAddress(target_ip_addr),target_ip_addr,options);
