...
public class VariableTlvAttribute extends TlvAttribute {
	...
	public static VariableTlvAttribute parseTlvAttribute(int type_size, int length_size, byte[] buf, int off) {
		int type=(int)Bytes.toInt(buf,off,type_size);
		off+=type_size;
		int len=(int)Bytes.toInt(buf,off,length_size);
		off+=length_size;		
	...
	public int getBytes(byte[] buf, int off) {
		Bytes.fromInt(type,buf,off,type_size);
		off+=type_size;
		Bytes.fromInt(len,buf,off,length_size);
		off+=length_size;
