...
import io.ipstack.net.ip4.IpAddressPrefix;
...
public class IpAddressUtils {
	...
	/** All possible IPv4 prefix masks */
	private static byte[][] IP4PREFIX_MASKS;

	/** All possible IPv6 prefix masks */
	private static byte[][] IP6PREFIX_MASKS;
	
	/** Gets an IPv4 prefix mask for a given prefix length.
	 * @param len the prefix length
	 * @return the prefix mask */
	public static byte[] prefixLengthToIp4Mask(int len) {
		if (IP4PREFIX_MASKS==null) IP4PREFIX_MASKS=new byte[33][];
		if (IP4PREFIX_MASKS[len]==null) IP4PREFIX_MASKS[len]=newPrefixMask(4,len);
		return IP4PREFIX_MASKS[len];			
	}
	
	/** Gets an IPv6 prefix mask for a given prefix length.
	 * @param len the prefix length
	 * @return the prefix mask */
	public static byte[] prefixLengthToIp6Mask(int len) {
		if (IP6PREFIX_MASKS==null) IP6PREFIX_MASKS=new byte[129][];
		if (IP6PREFIX_MASKS[len]==null) IP6PREFIX_MASKS[len]=newPrefixMask(16,len);
		return IP6PREFIX_MASKS[len];
	}
	 ...
	 * @return the prefix mask */
	private static byte[] newPrefixMask(int mask_size, int prefix_len) {
		byte[] mask=new byte[mask_size];
		for (int i=0; i<mask_size; i++) {
			int remainder=prefix_len-i*8;
	...
	/** Gets the prefix length from a IPv4 mask
	...
	/** Converts a {@link java.net.InetAddress} to a {@link io.ipstack.net.ip4.IpAddress}.
		...
		try {
			...
			if (split_index>=0) {
				...
				for (int i=0; i<leading_ss.length; i++) {
					//DefaultLogger.log(LoggerLevel.DEBUG,"Ip6Address: stringToBytes(): leading_ss["+i+"]: "+leading_ss[i]);
				...
				for (int i=0; i<tailing_ss.length; i++) {
					//DefaultLogger.log(LoggerLevel.DEBUG,"Ip6Address: stringToBytes(): tailing_ss["+(leading_ss.length+zero_len/2+i)+"]: "+tailing_ss[i]);
				...
				for (int i=0; i<8; i++) {
					//DefaultLogger.log(LoggerLevel.DEBUG,"Ip6Address: stringToBytes(): addr["+i+"]: "+ss[i]);
	...
	public static String bytesToStringIp6Address(byte[] buf, int off) {
		...
		for (int i=0; i<8; i++) {
			//DefaultLogger.log(LoggerLevel.DEBUG,"Ip6Address: bytesToString(): addr["+i+"]: "+addr[i]);
			if (addr[i]==0) {
		...
		//DefaultLogger.log(LoggerLevel.DEBUG,"Ip6Address: bytesToString(): zero index: "+zero_max_index+" ("+zero_max_count+")");
	...
	public static IpPrefix subnet(IpPrefix prefix, int subnet_len, long subnet_value) {
		byte[] addr=prefix.getPrefixAddress().getBytes();
		//byte[] addr=new byte[prefix.getPrefixAddress().length()];
		//prefix.getBytes(addr,0);
		if (subnet_len<prefix.getPrefixLength()) throw new RuntimeException("Subnet prefix length too short ("+subnet_len+"<"+prefix.getPrefixLength()+")");
		if (subnet_value>=(0x1<<(subnet_len-prefix.getPrefixLength()))) throw new RuntimeException("Subnet value too big ("+subnet_value+">="+(0x1<<(subnet_len-prefix.getPrefixLength()))+")");
		int len=subnet_len;
		while(len%8!=0) {
			subnet_value<<=1;
			len+=1;
	...
	public static IpAddressPrefix addressPrefix(IpPrefix prefix, long host_value) {
		...
		if (prefix instanceof Ip4Prefix) return new Ip4AddressPrefix(addr,0,prefix.getPrefixLength());
		if (prefix instanceof Ip6Prefix) return new Ip6AddressPrefix(addr,0,prefix.getPrefixLength());
		// else
		throw new RuntimeException("Unsupported prefix type: "+prefix.getClass().getSimpleName());		
	}
	...
	public static IpAddressPrefix addressPrefix(IpPrefix prefix, IpAddress host) {
		int len=prefix.getPrefixAddress().length();
	...
	/** Gets hardware address of a physical network interface.
	 * @param name name of the network interface
	 * @return the address */
	public static EthAddress getHardwareAddress(String name) {
		try {
			NetworkInterface ni=NetworkInterface.getByName(name);
			if (ni!=null) return new EthAddress(ni.getHardwareAddress());
		}
		catch (Exception e) {
		}
		return null;
	...
	/** Gets the index of a network interface.
	 * @param name name of the network interface
	 * @return the index */
	...
	/** Sorts a set of IP addresses.
	 * @param addrs the addresses
	 * @return the ordered list of addresses */
	public static <A extends IpAddress> ArrayList<A> sort(Collection<A> addrs) {
		HashMap<String,A> map=new HashMap<>();
		String[] straddrs=new String[addrs.size()];
		int i=0;
		for (A addr: addrs) {
			String str=Bytes.toHex(addr.getBytes());
			straddrs[i++]=str;
			map.put(str,addr);
		}
		Arrays.sort(straddrs);
		ArrayList<A> sorted_addrs=new ArrayList<>();
		for (String str: straddrs) sorted_addrs.add(map.get(str));
;		return sorted_addrs;
	}


	/** Sorts a set of IP addresses or names.
	 * @param addrs the addresses
	 * @param c the class of the IP addresses
	 * @return the ordered list of addresses */
	public static <A extends IpAddress> ArrayList<String> sort(Collection<String> addrs, Class<A> c) {
		...
		for (String addr: addrs) {
			String str;
			try {
				str=Bytes.toHex(c==Ip4Address.class? new Ip4Address(addr).getBytes() : new Ip6Address(addr).getBytes());
			}
			catch (Exception e) {
			}
			str='~'+addr; // after all hex chars
			straddrs[i++]=str;
			map.put(str,addr);
		}
		Arrays.sort(straddrs);
		ArrayList<String> sorted_addrs=new ArrayList<>();
		for (String str: straddrs) sorted_addrs.add(map.get(str));
;		return sorted_addrs;
	}
