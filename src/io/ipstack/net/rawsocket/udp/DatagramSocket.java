...
import io.ipstack.net.rawsocket.Socket;


/** UDP socket.
 * It is implemented on top of JNI {@link io.ipstack.net.rawsocket.Socket socket}.
 * <p>
...
public class DatagramSocket implements io.ipstack.net.socket.DatagramSocket {
	...
	public void receive(DatagramPacket p) throws IOException {
		...
		p.setLength(len);
		p.setAddress(InetAddress.getByAddress(src_addr));
		p.setPort(Bytes.toInt16(src_port));
		if (DEBUG) debug("receive(): src addr="+p.getAddress().getHostAddress()+", port="+p.getPort()+", len="+p.getLength());
	}
