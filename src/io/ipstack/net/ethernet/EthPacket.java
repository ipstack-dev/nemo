...
/** An Ethernet packet (IEEE 802.3).
  */
public class EthPacket extends DataPacket<EthAddress> {
	 ...
	 * <p>
	 * <b>Note:</b> It is currently set to 0 and no padding is added by the getBytes() method.
	 * It is supposed that the padding to the minimum length of 64 bytes (including the FCS) is enforced by the lower layer (the NIC).
	 * <br>
	 * Change it to 46 in order to force the method getBytes() to properly pad the frame to the minimum length of 64 bytes. */
	protected static final int MIN_PAYLOAD_LEN=0; // 46
	...
	public EthPacket(EthAddress src_addr, EthAddress dst_addr, int type, byte[] data) {
	...
	public EthPacket(EthAddress src_addr, EthAddress dst_addr, int type, byte[] data_buf, int data_off, int data_len) {
	...
	public int getBytes(byte[] buf, int off) {
		...
		if (data_len>=MIN_PAYLOAD_LEN) return 14+data_len;
			...
			Bytes.fill(buf,index+data_len,MIN_PAYLOAD_LEN-data_len,(byte)0x00);
	...
	public static EthPacket parseEthPacket(byte[] buf, int off, int len) {
		...
		//String dst_addr=Packet.toHex(buf,off,6);
		//String src_addr=Packet.toHex(buf,off+6,6);
	...
	public String toString() {
		...
		//return "src="+src_addr+", dst="+dst_addr+", proto="+type+", datalen="+getPayloadLength()+", payload="+Bytes.toHex(getBytes());
