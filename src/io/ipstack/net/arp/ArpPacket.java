 ...
 */
public class ArpPacket implements Packet<EthAddress> {
	...
	/** Source address */
	protected EthAddress src_addr;
	
	/** Destination address  */
	protected EthAddress dst_addr;
	...
	public ArpPacket(EthAddress src_addr, EthAddress dst_addr, int htype, int ptype, int hlen, int plen, int operation, byte[] sha, byte[] spa, byte[] tha, byte[] tpa) {
	...
	public ArpPacket(EthAddress src_addr, EthAddress dst_addr, int operation, EthAddress shaddr, Ip4Address spaddr, EthAddress thaddr, Ip4Address tpaddr) {
	...
	@Override
	public EthAddress getSourceAddress() {
		/*switch (ptype) {
	...
	@Override
	public EthAddress getDestAddress() {
		/*switch (ptype) {
	...
	public int getBytes(byte[] buf, int off) {
		Bytes.fromInt16(htype,buf,0);
		Bytes.fromInt16(ptype,buf,2);
		buf[off+4]=(byte)(hlen&0xff);
		buf[off+5]=(byte)(plen&0xff);
		Bytes.fromInt16(operation,buf,6);
	...
	public static ArpPacket parseArpPacket(EthAddress src_addr, EthAddress dst_addr, byte[] buf, int off, int len) {
		int htype=Bytes.toInt16(buf,off+0);
		int ptype=Bytes.toInt16(buf,off+2);
		int hlen=buf[off+4]&0xff;
		int plen=buf[off+5]&0xff;
		int operation=Bytes.toInt16(buf,off+6);
		byte[] sha=Bytes.copy(buf,off+8,hlen);
		byte[] spa=Bytes.copy(buf,off+14,plen);
		byte[] tha=Bytes.copy(buf,off+18,hlen);
		byte[] tpa=Bytes.copy(buf,off+24,plen);
	...
	public String toString() {
		...
		String sha_str=sha==null? null : htype==HARDWARE_TYPE_ETH? new EthAddress(sha).toString() : Bytes.toHex(sha);
		String tha_str=tha==null? null : htype==HARDWARE_TYPE_ETH? new EthAddress(tha).toString() : Bytes.toHex(tha);
		String spa_str=spa==null? null : ptype==PROTOCOL_TYPE_IP4? new Ip4Address(spa).toString() : Bytes.toHex(spa);
		String tpa_str=tpa==null? null : ptype==PROTOCOL_TYPE_IP4? new Ip4Address(tpa).toString() : Bytes.toHex(tpa);
