...
import io.ipstack.net.icmp4.message.IcmpParameterProblemMessage;
import io.ipstack.net.icmp4.message.IcmpRedirectMessage;
import io.ipstack.net.icmp4.message.IcmpSourceQuenchMessage;
...
import io.ipstack.net.icmp6.message.Icmp6RouterAdvertisementMessage;
import io.ipstack.net.icmp6.message.Icmp6RouterSolicitationMessage;
...
public class ProtocolAnalyzer {
	...
	protected static ArrayList<Packet<?>> explore(EthPacket eth_pkt) {
		...
		switch (type) {
			case EthPacket.ETH_ARP : list=new ArrayList<>(); list.add(ArpPacket.parseArpPacket(eth_pkt)); break;
	...
	protected static ArrayList<Packet<?>> explore(Ip4Packet ip_pkt) {
		...
		ArrayList<Packet<?>> list=null;
		switch (type) {
			case Ip4Packet.IPPROTO_ICMP : list=explore(new IcmpMessage(ip_pkt)); break;
	...
	protected static ArrayList<Packet<?>> explore(Ip6Packet ip_pkt) {
		...
		ArrayList<Packet<?>> list=null;
		switch (type) {
			case Ip6Packet.IPPROTO_ICMP6 : list=new ArrayList<>(); list.add(new Icmp6Message(ip_pkt)); break;
	...
	/** Gets the protocols encapsulated within a given ICMP message.
	  * @param icmp_msg the ICMP message
	  * @return the sequence of encapsulated packets, including the external ICMP message */
	protected static ArrayList<Packet<?>> explore(IcmpMessage icmp_msg) {
		int type=icmp_msg.getType();
		ArrayList<Packet<?>> list=new ArrayList<>();
		list.add(icmp_msg);
		switch (type) {
			case IcmpMessage.TYPE_Echo_Request : list.add(new IcmpEchoRequestMessage(icmp_msg)); break;
			case IcmpMessage.TYPE_Echo_Reply : list.add(new IcmpEchoReplyMessage(icmp_msg)); break;
			case IcmpMessage.TYPE_Destination_Unreachable : list.add(new IcmpDestinationUnreachableMessage(icmp_msg)); break;
			case IcmpMessage.TYPE_Redirect : list.add(new IcmpRedirectMessage(icmp_msg)); break;
			case IcmpMessage.TYPE_Source_Quench : list.add(new IcmpSourceQuenchMessage(icmp_msg)); break;
			case IcmpMessage.TYPE_Time_Exceeded : list.add(new IcmpTimeExceededMessage(icmp_msg)); break;
			case IcmpMessage.TYPE_Parameter_Problem : list.add(new IcmpParameterProblemMessage(icmp_msg)); break;
		}
		return list;
	}

	
	/** Gets the protocols encapsulated within a given ICMPv6 message.
	  * @param icmp_msg the ICMPv6 message
	  * @return the sequence of encapsulated packets, including the external ICMPv6 message */
	protected static ArrayList<Packet<?>> explore(Icmp6Message icmp_msg) {
		int type=icmp_msg.getType();
		ArrayList<Packet<?>> list=new ArrayList<>();
		list.add(icmp_msg);
		switch (type) {
			case Icmp6Message.TYPE_Echo_Request : list.add(new Icmp6EchoRequestMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Echo_Reply : list.add(new Icmp6EchoReplyMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Destination_Unreachable : list.add(new Icmp6DestinationUnreachableMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Time_Exceeded : list.add(new Icmp6TimeExceededMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Neighbor_Advertisement : list.add(new Icmp6NeighborAdvertisementMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Neighbor_Solicitation : list.add(new Icmp6NeighborSolicitationMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Router_Advertisement : list.add(new Icmp6RouterAdvertisementMessage(icmp_msg)); break;
			case Icmp6Message.TYPE_Router_Solicitation : list.add(new Icmp6RouterSolicitationMessage(icmp_msg)); break;
		}
		return list;
	}
	...
	public static ProtocolField analyze(EthPacket eth_pkt) {
		...
		switch (type) {
			...
			default : field.addSubField("payload",Bytes.toHex(buf,off,len));
		}	
		return field;
	}
	...
	public static ProtocolField analyze(Ip4Packet ip_pkt) {
		...
		field.addSubField("checksum","0x"+Bytes.fromInt16(ip_pkt.getChecksum()));
		...
		switch (ip_proto) {
			...
			default : field.addSubField("payload",Bytes.toHex(ip_pkt.getPayloadBuffer(),ip_pkt.getPayloadOffset(),ip_pkt.getPayloadLength()));
		}
		return field;
	}
