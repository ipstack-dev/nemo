...
import io.ipstack.net.tcp.option.Option;
...
public class TcpConnection {
	...
	private void debug(String str) {
		DefaultLogger.log(LOG_LEVEL,toString()+": "+str);
	}

	// CONSTANTS:
	
	static int MAX_RETRANSMISSIONS=5;
	static int RTO_K=4; // constant K used in RTO computation
	static long RTO_G=10; // clock granularity G (in milliseconds) used in RTO computation
	static float RTO_ALPHA=1/8; // alpha value used in RTO computation
	static float RTO_BETA=1/4; // beta value used in RTO computation
	static long RTO_MIN=1000; // minimum value of RTO
	static long RTO_MAX=16000; // maximum value of RTO
	
	static long SMSS=536; // Sender maximum segment size
	static long RMSS=536; // Receiver maximum segment size
	static int IW=4; // Initial value of cwnd, in segments
	/* If SMSS > 2190 bytes:
	...
	/** Retransmission timeout RTO (milliseconds) */
	long retransmission_to=RTO_MIN;
	
	/** Smoothed round-trip time SRTT (milliseconds) */
	float srtt=-1;
	
	/** Round-trip time variation RTTVAR (milliseconds) */
	float rttvar=-1;
	
	/** Closing timeout (milliseconds) */
	long closing_to=10000;
	...
	public SocketAddress geLocalSocketAddress() {
		return ci!=null? ci.getLocalSocketAddress() : new SocketAddress(local_ipaddr,local_port);
	}
	...
	public SocketAddress getRemoteSocketAddress() {
		return ci!=null? ci.getRemoteSocketAddress() : null;
	}
	...
	public void listen() throws IOException {
		...
		if (DEBUG) debug("listen(): port "+local_port+ " on "+tcp_layer.ip_layer);
		// else
	...
	void transmit(PacketTimer t) {
		long timeout=retransmission_to*(1<<t.getCounter());
		...
		*/// ##########################################
				...
				synchronized (snd_buffer) {
					for (int i=0; i<snd_buffer.size(); i++) {
						...
						if (lastSequenceNumber(t.getPacket())<ack) {
							...
							if (start_time>0) {
								long rtt=Clock.getDefaultClock().currentTimeMillis()-start_time;
								if (srtt<0) {
									srtt=rtt;
						            rttvar=rtt/2;
								}
								else {
									rttvar=(1-RTO_BETA)*rttvar+RTO_BETA*Math.abs(srtt-rtt);
									srtt=(1-RTO_ALPHA)*srtt+RTO_ALPHA*rtt;
								}
								retransmission_to=(long)(srtt+Math.max(RTO_G,RTO_K*rttvar));
								if (retransmission_to<RTO_MIN) retransmission_to=RTO_MIN;
								retransmission_to=RTO_MIN;
							}
							snd_buffer.remove(i);
							i--;
						}
		...
		synchronized (state) {
			...
			// DATA, FIN
				...
				// FIN
				if (tcp_pkt.hasFin()) {
					long fin_sqn=sqn+len;
					if (rcv_fsn<0) {
						long buffer_end=rcv_buffer!=null? rcv_buffer.end() : rcv_isn+1;
						if (fin_sqn<buffer_end) {
							if (DEBUG) debug("processIncomingPacket(): WARNING: FIN wrong sequence number ("+fin_sqn+"<"+rcv_buffer.end()+"): discarded");
							return;	
						}
						rcv_fsn=fin_sqn;
					...
					if (fin_sqn!=rcv_fsn) {
						if (DEBUG) debug("processIncomingPacket(): WARNING: FIN wrong sequence number ("+fin_sqn+"!="+rcv_fsn+"): discarded");
						return;
					}				
