...
/** TCP layer demultiplexes TCP connections, dispatching incoming TCP packets (segments) to the proper TCP connections. 
 */
public class TcpLayer {
	...
	/** Gets all server listeners.
	 * @return a table mapping server ports to corresponding listeners */
	public Hashtable<Integer,TcpLayerListener> getServerListeners() {
		synchronized (syn_listeners) {
			return (Hashtable<Integer,TcpLayerListener>)syn_listeners.clone();
		}
	}

	/** Gets all connection listeners.
	 * @return a table mapping connections to corresponding listeners */
	public Hashtable<ConnectionIdentifier,TcpLayerListener> getConnectionListeners() {
		synchronized (conn_listeners) {
			return (Hashtable<ConnectionIdentifier,TcpLayerListener>)conn_listeners.clone();
		}
	}
	...
	public IpAddress getSourceAddress(IpAddress dest_ipaddr) {
		return ip_layer.getSourceAddress((Ip4Address)dest_ipaddr);
	}
	...
	void send(TcpPacket tcp_pkt) {
		...
		if (tcp_pkt.getSourceAddress()==null) {
			IpAddress src_addr=getSourceAddress(tcp_pkt.getDestAddress());
			if (src_addr==null) {
