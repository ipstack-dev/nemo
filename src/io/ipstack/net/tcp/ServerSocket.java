 ...
 */
public class ServerSocket implements io.ipstack.net.socket.ServerSocket, java.io.Closeable {
	...
	public void close() throws IOException {
		if (isClosed()) return;
		closed=true;
		if (waiting_socket!=null) waiting_socket.close();
	}
	...
	public synchronized Socket accept() throws IOException {
		if (DEBUG) debug("accept()");
		if (isClosed()) throw new SocketException("Socket is closed");
		if (!isBound()) throw new SocketException("Socket is not bound yet");
		// else
		waiting_socket=new Socket(tcp_layer);
		waiting_socket.bind(new InetSocketAddress(local_inetaddr,local_port));
		waiting_socket.listen();
		Socket socket=waiting_socket;
		waiting_socket=null;
		return socket;
	}
