...
import io.ipstack.net.tcp.option.SackOption;
import io.ipstack.net.tcp.option.SackPermittedOption;
import io.ipstack.net.tcp.option.TimestampsOption;
import io.ipstack.net.tcp.option.TlvOption;
import io.ipstack.net.tcp.option.WindowScaleOption;
import io.ipstack.net.util.Checksum;


/** Transmission Control Protocol (RFC 793) segment unit.
 */
public class TcpPacket extends DataPacket<IpAddress> implements TransportPacket {
	...
	public static TcpPacket parseTcpPacket(IpAddress src_addr, IpAddress dst_addr, byte[] buf, int off, int len) {
		int src_port=Bytes.toInt16(buf,off+OFFSET_SP);
		int dst_port=Bytes.toInt16(buf,off+OFFSET_DP);
		long sqn=Bytes.toInt32(buf,off+OFFSET_SQN);
		long ack=Bytes.toInt32(buf,off+OFFSET_ACK);
		...
		int fwin=Bytes.toInt16(buf,off+OFFSET_WIN);
		int urg=Bytes.toInt16(buf,off+OFFSET_URG);
		...
		int checksum=Bytes.toInt16(buf,off+OFFSET_CSUM);
		if (checksum!=0) {		
	...
	public SocketAddress getSourceSocketAddress() {
		return new SocketAddress(getSourceAddress(),src_port);
	}

	@Override
	public void setSourceSocketAddress(SocketAddress soaddr) {
		setSourceAddress(soaddr.getIpAddress());
		src_port=soaddr.getPort();
	}

	@Override
	public SocketAddress getDestSocketAddress() {
		return new SocketAddress(getDestAddress(),dst_port);
	}

	@Override
	public void setDestSocketAddress(SocketAddress soaddr) {
		setDestAddress(soaddr.getIpAddress());
		dst_port=soaddr.getPort();
	}
	...
	public int getBytes(byte[] buf, int off) {
		...
		Bytes.fromInt16(src_port,buf,off+OFFSET_SP);
		Bytes.fromInt16(dst_port,buf,off+OFFSET_DP);	
		Bytes.fromInt32(sqn,buf,off+OFFSET_SQN);
		Bytes.fromInt32(ack>=0?ack:0,buf,off+OFFSET_ACK);
		int flags=(urg>=0?FLAG_URG:0)|(ack>=0?FLAG_ACK:0)|(psh?FLAG_PSH:0)|(rst?FLAG_RST:0)|(syn?FLAG_SYN:0)|(fin?FLAG_FIN:0);
		Bytes.fromInt16(flags,buf,off+OFFSET_FLAGS);
		int hlen=total_len-data_len;
		buf[off+OFFSET_HLEN]=(byte)((hlen/4)<<4);
		Bytes.fromInt16(window,buf,off+OFFSET_WIN);
		Bytes.fromInt16(0,buf,off+OFFSET_CSUM);
		Bytes.fromInt16(urg>=0?urg:0,buf,off+OFFSET_URG);
		if (options!=null) {
			int options_len=0;
		...
		Bytes.fromInt16(checksum,buf,off+OFFSET_CSUM);
		checksum_check=1;
		return total_len;
	}
	...
	public String toString() {	
		...
		if (options!=null) {
			for (Option opt:options) if (opt.getType()!=OPT_NO_OPERATION && opt.getType()!=OPT_END_OF_OPTION_LIST) sb.append(" ").append(opt);
		}
