 ...
 */
public class Socket implements io.ipstack.net.socket.Socket {
	...
	/** Gets the TCP layer.
	 * @return the underlying TCP layer */
	@Override
	...
	/** Binds the socket to a local address. */
	protected void bind(InetAddress inetaddr, int port) throws IOException {
	...
	public void listen() throws IOException {
		...
		if (connected) {
			remote_inetaddr=tcp_conn.getRemoteSocketAddress().getIpAddress().toInetAddress();
			remote_port=tcp_conn.getRemoteSocketAddress().getPort();			
		}
		else {
			if (closed) throw new IOException("Socket has been closed");
	...
	private void connect(InetAddress address, int port, int timeout) throws SocketException {
		...
		tcp_conn=new TcpConnection(tcp_layer,null,local_port,this_tcp_conn_listener);
		try {
			tcp_conn.connect(new io.ipstack.net.ip4.SocketAddress(address,port));
		}
		catch (IOException e) {
	...
	public InputStream getInputStream() throws IOException {
		...
		return new InputStream(){
			...
			public int read() throws IOException {
				...
				synchronized (receiver_buffer) {
					if (DEBUG) debug("getInputStream(): read(): available before: "+receiver_buffer.available());				
					while (receiver_buffer.available()==0) try { receiver_buffer.wait(); } catch (InterruptedException e) {}
					if (DEBUG) debug("getInputStream(): read(): available after: "+receiver_buffer.available());				
