 ...
 */
public class IcmpMessage implements Packet<Ip4Address> {
	...
	/** IP source address */
	protected Ip4Address src_addr;
	
	/** IP destination address */
	protected Ip4Address dst_addr;
	...
	protected IcmpMessage(Ip4Address src_addr, Ip4Address dst_addr, int type, int code) {
	...
	public IcmpMessage(Ip4Address src_addr, Ip4Address dst_addr, int type, int code, byte[] buf, int off, int len) {
		this.src_addr=src_addr;
		this.dst_addr=dst_addr;
		this.type=type;
		this.code=code;
	...
	public IcmpMessage(Ip4Address src_addr, Ip4Address dst_addr, byte[] buf, int off, int len) {
	 ...
	 * @param off the offset within the buffer
	 * @param len the length of the ICMP packet */
	public void init(Ip4Address src_addr, Ip4Address dst_addr, byte[] buf, int off, int len) {
		this.src_addr=src_addr;
		this.dst_addr=dst_addr;
		type=buf[off]&0xff;
		code=buf[off+1]&0xff;
		...
		if (checksum!=0x0000 && checksum!=0xffff) throw new RuntimeException("Wrong ICMP checksum: ICMP packet from "+src_addr+" to "+dst_addr+": "+Bytes.toHex(buf,off,len));
		// else
	...
	@Override
	public Ip4Address getDestAddress() {
		return dst_addr;
	}
	...
	public int getBytes(byte[] buf, int off) {
		...
		Bytes.fromInt16(checksum,buf,off+2);
		return icmp_len;
	}
	...
	public String toString() {
		if (type==TYPE_Echo_Request || type==TYPE_Echo_Reply) {
			int sqn=Bytes.toInt16(icmp_body,2);
