...
/** {@link it.unipr.netsec.ipstack.ip4.Ip4Layer IPv4 layer} that loads network configuration from a configuration file containing linux commands.
 */
public class LinuxIp4Layer extends Ip4Layer {
	 ...
	 * @throws IOException */
	public LinuxIp4Layer(NetConfiguration<Ip4Address,Ip4Packet> config) throws IOException {
		super(config.getNetInterfaces());
		ArrayList<Route<Ip4Address,Ip4Packet>> routes=config.getRoutes();
		RoutingTable<Ip4Address,Ip4Packet> rt=getRoutingTable();
		for (Route<Ip4Address,Ip4Packet> r: routes) rt.add(r);
	}
	...
	/*public LinuxIp4Layer(String config_file) throws IOException {
		...
		RoutingTable<Ip4Address,Ip4Packet> rt=getRoutingTable();
		for (Route<Ip4Address,Ip4Packet> r: temp_routing_table) rt.add(r);
	}*/
