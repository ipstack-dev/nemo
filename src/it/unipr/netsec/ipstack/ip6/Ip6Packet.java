...
import it.unipr.netsec.ipstack.ip4.IpPacketConstants;
	...
	public int getBytes(byte[] buf, int off) {
		...
		if (ext_hdrs!=null && ext_hdrs.size()>0) {
			...
			for (ExtensionHeader eh : ext_hdrs) {
				...
				if (DEBUG) SystemUtils.log(LoggerLevel.DEBUG,"Ip6Packet: getBytes(): ExtHdr "+eh.getHeaderType()+": "+Bytes.toHex(buf,index,len));
				index+=len;
			}
		}
		// payload
