...
public class OspfPacket implements Packet {
	...
	public OspfPacket(Address src_addr, Address dst_addr, byte[] buf, int off, int len) {
		...
		type=buf[off+1]&0xff;
		int length=Bytes.toInt16(buf,off+2);
		if (length!=len) throw new RuntimeException("Wrong OSPF packet length: OSPF packet from "+src_addr+" to "+dst_addr+": "+Bytes.toHex(buf,off,len));
		router_id=new Ip4Address(buf,off+4);
		area_id=new Ip4Address(buf,off+8);
		authentication=Bytes.toInt(buf,off+16,8);
		int checksum=Checksum.checksum(buf,off,len) & 0xffff;
		if (checksum!=0x0000 && checksum!=0xffff) throw new RuntimeException("Wrong OSPF checksum: OSPF packet from "+src_addr+" to "+dst_addr+": "+Bytes.toHex(buf,off,len));
		// else
