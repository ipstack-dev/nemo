...
public class LSALink {
	...
	public LSALink(byte[] buf, int off) {
		this(0xff&buf[off+8],new Ip4AddressPrefix(buf,off,IpAddressUtils.maskToPrefixLength(buf,off+4)),Bytes.toInt16(buf,off+10));
		...
		for (int i=0; i<tos_num; i++) {
			...
			tos_metric[i]=Bytes.toInt16(buf,off+14+i*4);
		}
	}
	...
	public int getBytes(byte[] buf, int off) {
		...
		System.arraycopy(link_addr_prefix.getPrefix().getPrefixMask(),0,buf,off+4,4);
		buf[off+8]=(byte)type;
		int tos_len=tos!=null?tos.length:0;
		buf[off+9]=(byte)tos_len;
		Bytes.fromInt16(metric,buf,off+10);
		for (int i=0; i<tos_len; i++) {
			buf[off+12+i*4]=(byte)tos[i];
			Bytes.fromInt16(tos_metric[i],buf,off+14+i*4);
		}
		return 12+4*tos_len;
	}
