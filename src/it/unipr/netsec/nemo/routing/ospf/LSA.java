...
public class LSA {
	...
	public LSA(byte[] buf, int off) {
		age=Bytes.toInt16(buf,off);
		options=0xff&buf[off+2];
		type=0xff&buf[off+3];
		id=Bytes.toInt32(buf,off+4);
		router=new Ip4Address(buf,off+8);
		sqn=Bytes.toInt32(buf,off+12);
		int len=Bytes.toInt16(buf,off+18);
		body=new byte[len];
	...
	public int getBytes(byte[] buf, int off) {
		Bytes.fromInt16(age,buf,off);
		buf[off+2]=(byte)options;
		buf[off+3]=(byte)type;
		Bytes.fromInt32(id,buf,off+4);
		router.getBytes(buf,off+8);
		Bytes.fromInt32(sqn,buf,off+12);
		Bytes.fromInt16(20+body.length,buf,off+18);
