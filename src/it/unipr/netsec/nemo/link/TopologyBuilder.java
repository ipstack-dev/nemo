...
public class TopologyBuilder {
	...
	public static <A extends IpAddress, P extends Packet<A>> Network<Node<A,P>,IpLink<A,P>> linearIpNetworkAlt(int n, long bit_rate, IpPrefix net_prefix) {
		// create all links
		IpLink<A,P>[] links=new IpLink[n+1];
		...
		for (int i=0; i<n; i++) {
			if (net_prefix instanceof Ip6Prefix) {
				routers[i]=new Ip6Router((IpLink<Ip6Address,Ip6Packet>)links[i],(IpLink<Ip6Address,Ip6Packet>)links[i+1]);
				((Ip6Router)routers[i]).setDynamicRouting(dynamic_routing);
			}
			else {
				routers[i]=new Ip4Router((IpLink<Ip4Address,Ip4Packet>)links[i],(IpLink<Ip4Address,Ip4Packet>)links[i+1]);
		...
		Network<Node<A,P>,IpLink<A,P>> network=new Network<>();
		for (Node<A,P> router : routers) network.addNode(router);
		int count=0;
		for (IpLink<A,P> link : links) network.addLink("link-"+(count++),link);		
		return network;
	}
	 ...
	 * @param net_prefix network prefix
	 * @return the new network */
	public static <A extends IpAddress, P extends Packet<A>> Network<Node<A,P>,IpLink<A,P>> linearIpNetwork(int n, long bit_rate, IpPrefix net_prefix) {
		IpPrefix loopback_prefix=net_prefix instanceof Ip4Prefix? CORE_LOCAL_LINK_PREFIX4 : CORE_LOCAL_LINK_PREFIX6;
		int len=net_prefix.getPrefixLength()+IpAddressUtils.ceilLog2(n+2);
		...
		for (int i=0; i<n; i++) {
			...
			DataLinkInterface<A,P> ni0=new DataLinkInterface<A,P>(links[i],(A)IpAddressUtils.addressPrefix(links[i].getPrefix(),i==0? 1 : 2));
			DataLinkInterface<A,P> ni1=new DataLinkInterface<A,P>(links[i+1],(A)IpAddressUtils.addressPrefix(links[i+1].getPrefix(),1));
			IpAddress name=IpAddressUtils.addressPrefix(loopback_prefix,i+1);
			if (net_prefix instanceof Ip6Prefix) routers[i]=new Ip6Router((Ip6Address)name,(NetInterface<Ip6Address,Ip6Packet>)ni0,(NetInterface<Ip6Address,Ip6Packet>)ni1);
			else routers[i]=new Ip4Router((Ip4Address)name,(NetInterface<Ip4Address,Ip4Packet>)ni0,(NetInterface<Ip4Address,Ip4Packet>)ni1);
			links[i].addRouter((A)ni0.getAddress());
			links[i+1].addRouter((A)ni1.getAddress());
	 ...
	 * @param net_prefix network prefix
	 * @return the new network */
	public static <A extends IpAddress, P extends Packet<A>> Network<Node<A,P>,IpLink<A,P>> manhattanIpNetwork(int n, int m, long bit_rate, IpPrefix net_prefix) {
		IpPrefix loopback_prefix=net_prefix instanceof Ip4Prefix? CORE_LOCAL_LINK_PREFIX4 : CORE_LOCAL_LINK_PREFIX6;
		// Links:
		int num_of_links=2*n*m+n+m; // 2n^2 +2n
		int len=net_prefix.getPrefixLength()+IpAddressUtils.ceilLog2(num_of_links+1);

		IpLink<A,P>[] links=new IpLink[num_of_links];
		...
		for (int i=0; i<n; i++) {
			for (int j=0; j<m; j++) {
				IpLink<A,P> link0=links[i*(2*m+1)+j]; // north
				IpLink<A,P> link1=links[i*(2*m+1)+m+j]; // west
				IpLink<A,P> link2=links[i*(2*m+1)+m+j+1]; // east
				IpLink<A,P> link3=links[i*(2*m+1)+2*m+1+j]; // south
				DataLinkInterface<A,P>[] eth=new DataLinkInterface[4];
				eth[0]=new DataLinkInterface<A,P>(link0,(A)IpAddressUtils.addressPrefix(link0.getPrefix(),1)); // north
				eth[1]=new DataLinkInterface<A,P>(link1,(A)IpAddressUtils.addressPrefix(link1.getPrefix(),2)); // west
				eth[2]=new DataLinkInterface<A,P>(link2,(A)IpAddressUtils.addressPrefix(link2.getPrefix(),3)); // east
				eth[3]=new DataLinkInterface<A,P>(link3,(A)IpAddressUtils.addressPrefix(link3.getPrefix(),4)); // south
				//String name="R_"+i+","+j;
				A loopback_addr=(A)IpAddressUtils.addressPrefix(loopback_prefix,i*m+j+1);
				if (net_prefix instanceof Ip6Prefix) routers[i*m+j]=new Ip6Router((Ip6Address)loopback_addr,Arrays.asList(((NetInterface<Ip6Address,Ip6Packet>[])eth)));
				else routers[i*m+j]=new Ip4Router((Ip4Address)loopback_addr,Arrays.asList(((NetInterface<Ip4Address,Ip4Packet>[])eth)));
				link0.addRouter(eth[0].getAddress());
				link1.addRouter(eth[1].getAddress());
				link2.addRouter(eth[2].getAddress());
				link3.addRouter(eth[3].getAddress());
				RoutingTable<A,P> rt=(RoutingTable)routers[i*m+j].getRoutingFunction();
				rt.removeAll();
				rt.add(new Route<A,P>((A)link0.getPrefix(),null,eth[0]));
				rt.add(new Route<A,P>((A)link1.getPrefix(),null,eth[1]));
				rt.add(new Route<A,P>((A)link2.getPrefix(),null,eth[2]));
				rt.add(new Route<A,P>((A)link3.getPrefix(),null,eth[3]));
				for (int k=0; k<num_of_links; k++) {
					...
					if (gw!=null) rt.add(new Route<A,P>((A)dest_prefix,(A)gw,eth_out));			
				}
			}
		}
		Network<Node<A,P>,IpLink<A,P>> network=new Network<>();
	...
	/** Creates a tree topology with (m^n)-1 routers and (m^(n+1))-2 links, where n is the tree height and m is the tree degree.
	 ...
	 * All core links use the same prefix; router addresses are assigned as globally unique from that prefix.
	 ...
	 * @param bit_rate link bit rate
	 * @return the new network */
	public static <A extends IpAddress, P extends Packet<A>> Network<Node<A,P>,IpLink<A,P>> treeIpCoreNetwork(int degree, int height, long bit_rate, IpPrefix net_prefix) {
		List<Node<A,P>> routers=new ArrayList<>();
		List<IpLink<A,P>> access_links=new ArrayList<>();
		List<IpLink<A,P>> core_links=new ArrayList<>();
		IpPrefix core_link_prefix=net_prefix instanceof Ip4Prefix? CORE_LOCAL_LINK_PREFIX4 : CORE_LOCAL_LINK_PREFIX6; // prefix used for core links
		IpLink<A,P> core_addresses=new IpLink<A,P>(core_link_prefix); // virtual link used only for assigning unique addresses to all router interfaces
		
		IpLink<A,P> root_link=new IpLink<A,P>(core_link_prefix); // upper link toward the root (unused for the root)
		...
		createSubTree(routers,access_links,core_links,core_addresses,root_addr,root_link,node_addr,net_prefix,height,degree,bit_rate);
		
		List<IpLink<A,P>> links=core_links;
		links.addAll(access_links);
		Network<Node<A,P>,IpLink<A,P>> network=new Network<>();
		for (Node<A,P> router : routers) network.addNode(router);
		int count=0;
		for (IpLink<A,P> link : links) network.addLink("link-"+(count++),link);		
	...
	private static <A extends IpAddress, P extends Packet<A>> void createSubTree(List<Node<A,P>> all_routers, List<IpLink<A,P>> access_links, List<IpLink<A,P>> core_links, IpLink core_addresses, IpAddress root_addr, IpLink<A,P> root_link, IpAddress node_addr, IpPrefix net_prefix, int more_layers, int degree, long bit_rate) {
		...
		IpLink<A,P>[] subnet_links=new IpLink[num_leaves];
		DataLinkInterface<A,P>[] link_interfaces=new DataLinkInterface[num_leaves];
		if (more_layers>0) {
			for (int i=0; i<num_leaves; i++) {
				subnet_prefixes[i]=IpAddressUtils.subnet(net_prefix,net_prefix.getPrefixLength()+subnet_bits,i);
				subnet_links[i]=new IpLink<A,P>(bit_rate,core_addresses.getPrefix());
				IpAddress addr_i=core_addresses.nextAddressPrefix();
				leaf_addrs[i]=core_addresses.nextAddressPrefix();
				link_interfaces[i]=new IpLinkInterface<A,P>(subnet_links[i],(A)addr_i);
		...
		Node router=net_prefix instanceof Ip6Prefix? new Ip6Router((Ip6Address)node_addr,(NetInterface<Ip6Address,Ip6Packet>[])link_interfaces) : new Ip4Router((Ip4Address)node_addr,(NetInterface<Ip4Address,Ip4Packet>[])link_interfaces);
		IpLinkInterface<A,P> root_interface=new IpLinkInterface(root_link,node_addr);
		...
		if (more_layers==0) {
			for (int i=0; i<num_leaves; i++) subnet_links[i].addRouter((A)link_interfaces[i].getAddress());
		}
	}
