 ...
 */
public class EthLinkInterface extends DataLinkInterface<EthAddress,EthPacket> {
	...
	public EthLinkInterface(DataLink<EthAddress,EthPacket> link) {
		this(link,new EthAddress(Bytes.concat(OUI,Random.nextBytes(6-OUI.length))));
	}
	
	/** Creates a new interface.
	 * @param link the link to be attached to
	 * @param addr the interface address */
	public EthLinkInterface(DataLink<EthAddress,EthPacket> link, EthAddress addr) {
		super(link,addr);
	}
