...
/** Simple HTTP server.
 * It passes all HTTP requests, by mean of a corresponding {@link HttpRequestHandle}, to a given {@link HttpServerListener}.
 */
public class HttpServer {
	 ...
	 * @throws IOException */
	public HttpServer(TcpLayer tcp_layer, int server_port, HttpServerListener listener) throws IOException {
		server_socket=new ServerSocket(new it.unipr.netsec.ipstack.tcp.ServerSocket(tcp_layer,server_port));
	...
	public HttpServer(int server_port, HttpServerListener listener) throws IOException {
		server_socket=new ServerSocket(new java.net.ServerSocket(server_port));
		this.listener=listener;
		start();
	}
