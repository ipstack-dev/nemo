...
public class DhcpClient extends Thread implements TimerListener {
	...
	public void client() {
		...
		try {
			long r=(long)(stack.desync_start_time*DhcpStack.rand.nextDouble());
			Thread.sleep(r);
		}
		...
		while(!stop) {
			...
			if (message.isReply() && (message.stringXid().equals(Mangle.toHex(xid)) || message.isDhcpAdv())) {
