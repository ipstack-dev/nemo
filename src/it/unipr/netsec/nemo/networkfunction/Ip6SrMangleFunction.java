	...
	public int processPacket(byte[] buf, int len) {
		...
		if (version==6) {
			...
			if (srf==null && ip6_pkt.hasExtHdr(ExtensionHeader.DST_OPTIONS_HDR)) {
				...
				debug("Destination Options header: "+Bytes.toHex(doh.getBytes()));
				//debug("Option: "+Bytes.toHex(doh.getOptions()[0].getBytes()));
				debug("Option value: "+Bytes.toHex(doh.getOptions()[0].getValue()));
