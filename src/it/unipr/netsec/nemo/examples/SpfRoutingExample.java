...
public class SpfRoutingExample {
	...
	public static void main(String[] args) {
		...
		IpLink<Ip4Address,Ip4Packet>[][] h_links=new IpLink[n][m+1];
		...
		IpLink<Ip4Address,Ip4Packet>[][] v_links=new IpLink[n+1][m];
		...
		for (int i=0; i<n; i++) for (int j=0; j<m; j++) {
			routers[i][j]=new Ip4Router(h_links[i][j],h_links[i][j+1],v_links[i][j],v_links[i+1][j]);
